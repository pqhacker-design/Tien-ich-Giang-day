...
with col1:
    with st.container(border=True):
        ...
        st.write("Tự động phân tích giáo án Word (.docx) và chèn nội dung Năng lực số - Năng lực AI theo đúng các vị trí hoạt động dạy học thực tế.")
...
with col4:
    with st.container(border=True):
        ...
        if st.button("🚀 Mở ứng dụng Ra đề kiểm tra", key="btn_ra_de_kt", use_container_width=True):
            st.switch_page("pages/4_📊_Ra_đề_kiểm_tra.py")

with col5:
    with st.container(border=True):
        st.markdown("#### ✍️ 5. Trợ lý viết SKKN")
        ...
        if st.button("🚀 Mở ứng dụng Viết SKKN", key="btn_sang_kien_kinh_nghiem", use_container_width=True):
            st.switch_page("pages/5_✍️_Viết_Sáng_kiến_kinh_nghiệm.py")

with col6:
    with st.container(border=True):
        st.markdown("#### 🎓 6. Gia sư AI - Hỗ trợ HS học tập")
        ...
        if st.button("🚀 Mở ứng dụng Gia sư", key="btn_gia_su_ai", use_container_width=True):
            st.switch_page("pages/6_🎓_Gia_Sư_AI.py")
...
with col7:
    with st.container(border=True):
        st.markdown("#### 🔄 7. Chuyển đổi file .Pdf thành Word (.docx)")
        ...
        if st.button("🚀 Mở ứng dụng Pdf to Word", key="btn_pdf_to_word", use_container_width=True):
            st.switch_page("pages/7_🔄_Chuyển_đổi_Pdf_to_Word.py")
with col8:
    with st.container(border=True):
        st.markdown("#### 📐 8. Trợ lý giúp vẽ hình Hình học")

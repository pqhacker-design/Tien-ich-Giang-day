...
def generate_shuffled_bundle(original_data, start_code, num_codes):
    ...
    # ĐƯA ĐỀ GỐC VÀO DANH SÁCH ĐÓNG GÓI ĐẦU TIÊN
    bundle["ĐỀ GỐC"] = copy.deepcopy(original_data)
...
# ==========================================
# CÁC HÀM TRÍCH XUẤT ĐỌC AI LƯỢT 1 & 2
# ==========================================
...
def generate_step2_questions(model, config, matrix_data, subject_rule):
    ...
    - Phần II (Trắc nghiệm Đúng/Sai): {config['num_tn_dung_sai']} câu.
    ...
    - Hãy điều phối độ phức tạp, câu hỏi ngữ cảnh bám sát mức độ khó này.
    ...
    - Sử dụng ký tự Unicode toán học trực tiếp trong văn bản trơn (Ví dụ: x², cm², (x + 1)/(x - 3), ΔABC, ∽, ⊥, //, ≠, ·, 30°, G₁).

    BẮT BUỘC TRẢ VỀ JSON NGUYÊN BẢN CÓ CẤU TRÚC:
    {{
      "de_kiem_tra": {{
        "trac_nghiem_4_lua_chon": [
          {{"id": 1, "cau_hoi": "Nội dung...", "A": "A", "B": "B", "C": "C", "D": "D"}}
        ],
        "trac_nghiem_dung_sai": [
          {{"id": 1, "cau_hoi": "Ngữ cảnh...", "cac_y": {{"a": "Ý a", "b": "Ý b", "c": "Ý c", "d": "Ý d"}}}}
        ],
        "trac_nghiem_tra_loi_ngan": [
          {{"id": 1, "cau_hoi": "Nội dung..."}}
        ],
        "tu_luan": [
          {{"id": 1, "cau_hoi": "Nội dung..."}}
        ]
...
# ==========================================
# HÀM XUẤT ĐỊNH DẠNG FILE WORD (.DOCX)
# ==========================================
def build_single_docx(config, data, code_label, include_matrix=True):
    ...
    # 1. TIÊU ĐỀ ĐỀ THI ĐỘNG THEO HÌNH THỨC KIỂM TRA CHỌN TỪ UI
    ...
    # 2. XỬ LÝ MA TRẬN PHÂN BỔ CÓ HÀNG TỔNG CÂU VÀ TỔNG ĐIỂM
    ...
    if list_4lc:
        ...
        p_hdr1.add_run("Mỗi câu hỏi thí sinh chỉ chọn một phương án.")
        for q in list_4lc:
            ...
            for char in ['A', 'B', 'C', 'D']:
                p_opts.add_run(f"{char}. ").bold = True
                add_math_run_to_paragraph(p_opts, q.get(char, ''))
                p_opts.add_run("    ")
    ...
    if list_ds:
        ...
        p_hdr2.add_run("Trong mỗi ý a), b), c), d) ở mỗi câu, thí sinh chọn đúng hoặc sai.")
        for q in list_ds:
            p_q = doc.add_paragraph()
            p_q.add_run(f"Câu {q.get('id', '')}: ").bold = True
            add_math_run_to_paragraph(p_q, q.get('cau_hoi', ''))
    ...
    if list_tln:
        ...
        p_hdr3.add_run("Thí sinh điền kết quả ngắn gọn vào ô tương ứng.")
        for q in list_tln:
            p_q = doc.add_paragraph()
            p_q.add_run(f"Câu {q.get('id', '')}: ").bold = True
            add_math_run_to_paragraph(p_q, q.get('cau_hoi', ''))
    ...
    if da.get('trac_nghiem_4_lua_chon'):
        ...
        ans_t1 = doc.add_table(rows=1, cols=2); ans_t1.style = 'Table Grid'
        ans_t1.rows[0].cells[0].text = "Câu"; ans_t1.rows[0].cells[1].text = "Đáp án"
        for q_id, val in da['trac_nghiem_4_lua_chon'].items():
            rc = ans_t1.add_row().cells; rc[0].text = f"Câu {q_id}"; rc[1].text = str(val)
            
    if da.get('trac_nghiem_dung_sai'):
        doc.add_paragraph().add_run("\nĐáp án Phần II:").bold = True
        ans_t2 = doc.add_table(rows=1, cols=2); ans_t2.style = 'Table Grid'
        ans_t2.rows[0].cells[0].text = "Câu"; ans_t2.rows[0].cells[1].text = "Hướng dẫn ý"
        for q_id, val_dict in da['trac_nghiem_dung_sai'].items():
            rc = ans_t2.add_row().cells; rc[0].text = f"Câu {q_id}"
            rc[1].text = ", ".join([f"{k}: {v}" for k, v in val_dict.items()]) if isinstance(val_dict, dict) else str(val_dict)
            
    if da.get('trac_nghiem_tra_loi_ngan'):
        doc.add_paragraph().add_run("\nĐáp án Phần III:").bold = True
        ans_t3 = doc.add_table(rows=1, cols=2); ans_t3.style = 'Table Grid'
        ans_t3.rows[0].cells[0].text = "Câu"; ans_t3.rows[0].cells[1].text = "Kết quả"
        for q_id, val in da['trac_nghiem_tra_loi_ngan'].items():
            rc = ans_t3.add_row().cells; rc[0].text = f"Câu {q_id}"; rc[1].text = str(val)

    if da.get('tu_luan'):
        doc.add_paragraph().add_run("\nHướng dẫn chi tiết Tự luận:").bold = True
...
# ==========================================
# THIẾT KẾ KHUNG ĐIỀU KHIỂN UI STREAMLIT
# ==========================================
st.markdown('<div class="main-title">Trợ Lý Thiết Kế Đề Thi Đa Năng</div>', unsafe_allow_html=True)
...
tab1, tab2, tab3 = st.tabs(["📋 1. Thông tin & Số lượng", "📊 2. Lập Ma trận & Sinh đề", "📥 3. Xuất bản File Word"])

with tab1:
    col1, col2 = st.columns(2)
    with col1:
        st.markdown('<div class="section-header">Lựa chọn bộ môn & Hình thức</div>', unsafe_allow_html=True)
        subject = st.selectbox("Chọn môn học:", list(SUBJECTS_CONFIG.keys()))
        grade = st.selectbox("Khối lớp:", [str(i) for i in range(1, 13)], index=7)
        exam_type = st.selectbox("Hình thức kiểm tra:", ["Giữa học kỳ I", "Cuối học kỳ I", "Giữa học kỳ II", "Cuối học kỳ II", "Khảo sát chất lượng"])
        ...
        diff_presets = {
            ...
            "Phân hóa mạnh (Bồi dưỡng học sinh giỏi)": {"nb": 20, "th": 30, "vd": 30, "vdc": 20}
        }
        level_choice = st.selectbox("Độ khó mục tiêu của đề thi:", list(diff_presets.keys()), index=1)
        target_ratios = diff_presets[level_choice]

    with col2:
        st.markdown('<div class="section-header">Số câu hỏi thành phần theo phom mới</div>', unsafe_allow_html=True)
        ...
        st.markdown('---')
        code_choice = st.selectbox("Số lượng mã đề đảo:", [1, 2, 4, 6, 8, "Tùy chọn"], index=2)
        num_codes = st.number_input("Số mã đề đảo thực tế:", min_value=1, max_value=24, value=4) if code_choice == "Tùy chọn" else int(code_choice)
        code_prefix = st.text_input("Mã đề bắt đầu từ:", value="101")

with tab2:
    st.markdown('<div class="section-header">Điều chỉnh tỉ lệ nhận thức</div>', unsafe_allow_html=True)
    ...
    if total_ratio != 100:
        st.warning(f"⚠️ Tổng tỷ lệ đạt {total_ratio}%. Thầy cô vui lòng chỉnh lại cho đúng 100%.")
        
    topics_list = st.text_area("Danh sách các chủ đề kiến thức (Mỗi dòng một chủ đề):", value="Chủ đề trọng tâm chương học\nChủ đề nâng cao bổ trợ liên quan")
    st.markdown("---")
    
    col_b1, col_b2 = st.columns(2)
    with col_b1:
        if st.button("📊 BƯỚC 1: LẬP MA TRẬN & ĐẶC TẢ"):
            if total_ratio != 100: st.error("Tổng tỷ lệ phải bằng 100%.")
            else:
                config_pkg = {
                    ...
                    "nb_ratio": nb_ratio, "th_ratio": th_ratio, "vd_ratio": vd_ratio, "vdc_ratio": vdc_ratio, "difficulty": level_choice
                }
                with st.spinner("AI đang tính toán khung đặc tả..."):
                    try:
    ...
    if st.session_state.step1_data:
        with col_b2:
            if st.button("🔥 BƯỚC 2: SOẠN CÂU HỎI & ĐẢO ĐỀ"):
                config_pkg = {
                    "subject": subject, "grade": grade, "num_tn_4_lua_chon": num_tn_4_lua_chon, 
                    "num_tn_dung_sai": num_tn_dung_sai, "num_tn_tra_loi_ngan": num_tn_tra_loi_ngan, "num_tl": num_tl,
                    "exam_type": exam_type, "duration": duration, "school_year": school_year, "difficulty": level_choice,
                    "nb_ratio": nb_ratio, "th_ratio": th_ratio, "vd_ratio": vd_ratio, "vdc_ratio": vdc_ratio
                }
                with st.spinner("AI đang soạn thảo chi tiết các phần câu hỏi..."):
                    try:
                        ...
                        st.success(f"🎉 Đã hoàn thành! Đã tạo ĐỀ GỐC và {num_codes} mã đề hoán vị thành công.")
                    except Exception as e: st.error(f"Lỗi: {e}")

with tab3:
    if st.session_state.generated_data is None:
        st.info("Hệ thống đang ở trạng thái chờ dữ liệu câu hỏi từ Bước 2.")
    else:
        config_pkg = { 
        ...
        export_mode = st.radio("Hình thức xuất bản bộ đề:", 
                               ["Tải file đơn lẻ (Tùy chọn tải Đề gốc hoặc Mã đề đảo)", "Nén tất cả mã đề (Bao gồm cả Đề gốc) vào file ZIP", "Gộp chung tất cả mã đề vào 1 file Word duy nhất"])
        
        if export_mode == "Tải file đơn lẻ (Tùy chọn tải Đề gốc hoặc Mã đề đảo)":
            sel_code = st.selectbox("Chọn bản đề cần xuất bản:", list(st.session_state.multi_codes_data.keys()))
            ...
            with zipfile.ZipFile(zip_buffer, "a", zipfile.ZIP_DEFLATED, False) as zip_file:
                for c_code, c_data in st.session_state.multi_codes_data.items():
                    ...
                    zip_file.writestr(f"De_{subject}_Lop{grade}_Code_{c_code}.docx", d_buf.getvalue())
            zip_buffer.seek(0)

...
with tab_game:
    ...
    current_class = db.get("currentClass", class_list[0])
...
# ==========================================
# TAB 2: CẤU HÌNH & QUẢN LÝ (PYTHON/STREAMLIT)
# ==========================================
with tab_config:
    st.header("⚙️ Quản Lý Cấu Hình & Danh Sách Lớp Học")

    col_cfg1, col_cfg2 = st.columns([1, 1], gap="large")

    with col_cfg1:
        st.subheader("🏫 Chọn & Quản Lý Lớp Học")
        ...
        with col_sel:
            ...
            if selected_class != db["currentClass"]:
                db["currentClass"] = selected_class
                save_database(db)
                st.rerun()

        with col_del:
            st.write(" ") # Căn dòng với selectbox
            st.write(" ")
            if st.button("🗑️ Xóa lớp", help=f"Xóa vĩnh viễn lớp {db['currentClass']}"):
                curr = db["currentClass"]
                if curr in db["classes"]:
                    del db["classes"][curr]
                    rem = list(db["classes"].keys())
                    db["currentClass"] = rem[0] if rem else "6A1"
                    if "6A1" not in db["classes"] and not rem:
                        db["classes"]["6A1"] = []
                    save_database(db)
                    st.toast(f"❌ Đã xóa lớp {curr}!", icon="🗑️")
                    st.rerun()
        ...
        with st.form("form_create_class", clear_on_submit=True):
            ...
            if st.form_submit_button("Tạo lớp") and new_c.strip():
                ...
                if c_name not in db["classes"]:
                    ...
                    save_database(db)
                    st.success(f"Đã tạo thành công lớp {c_name}!")
                    st.rerun()

        st.divider()

        # Import / Export
        st.subheader("💾 Tệp Dữ Liệu Cá Nhân")
        st.caption("Giúp lưu trữ riêng tư, không lo bị mất dữ liệu.")
        
        db_ready = json.dumps(db, ensure_ascii=False, indent=4)
        st.download_button(
            label="📥 Tải toàn bộ dữ liệu về máy (.json)",
            data=db_ready,
    ...
    with col_cfg2:
            ...
            with col_ex1:
                if st.button("➕ Thêm nối tiếp", use_container_width=True):
                    if excel_paste.strip():
                        lines = [l.strip() for l in excel_paste.split("\n") if l.strip()]
                        start_id = len(db["classes"][db["currentClass"]]) + 101
                        for idx, name in enumerate(lines):
                            db["classes"][db["currentClass"]].append({
                                "id": str(start_id + idx), "name": name, "class": db["currentClass"], "gender": "Nam", "avatar": "", "score": 0
                            })
                        save_database(db)
                        st.success(f"Đã thêm {len(lines)} học sinh!")
                        st.rerun()
            with col_ex2:
                if st.button("🔄 Ghi đè lớp", use_container_width=True):
                    if excel_paste.strip():
                        lines = [l.strip() for l in excel_paste.split("\n") if l.strip()]
                        db["classes"][db["currentClass"]] = []
                        for idx, name in enumerate(lines):
                            db["classes"][db["currentClass"]].append({
                                "id": str(101 + idx), "name": name, "class": db["currentClass"], "gender": "Nam", "avatar": "", "score": 0
                            })
                        save_database(db)
                        st.success(f"Đã làm mới lớp với {len(lines)} học sinh!")
                        st.rerun()
        ...
        with st.form("form_add_student", clear_on_submit=True):
            ...
            if st.form_submit_button("🔥 Xác Nhận Thêm") and s_name.strip():
                ...
                cur_list.append({
                    "id": str(len(cur_list) + 101), "name": s_name.strip(), "class": db["currentClass"], "gender": s_gender, "avatar": "", "score": 0
                })
                save_database(db)
                st.success(f"Đã thêm học sinh {s_name}!")
                st.rerun()

        # Bảng danh sách chi tiết
        current_students = db["classes"].get(db["currentClass"], [])
        if current_students:
            ...
            for idx, s in enumerate(current_students):
                ...
                if c_item3.button("🗑️ Xóa", key=f"cfg_del_{s['id']}_{idx}"):
                    current_students.pop(idx)
                    save_database(db)
                    st.rerun()
        else:
            st.info("Lớp học chưa có dữ liệu học sinh.")
